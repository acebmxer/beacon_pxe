...
Themed with the fixed ANSI colour slots so it renders correctly in plain VGA text
mode (no framebuffer console required); the palette follows the web UI's
light/dark setting.
"""
import logging
import shutil
...
# Per-theme colour pairs, built from the 16 fixed ANSI slots that iPXE renders
# correctly in plain VGA text mode (no framebuffer required):
#   0=black 1=red 2=green 3=yellow 4=blue 5=magenta 6=cyan 7=white
#   8-15 = bright variants of the above.
# We deliberately DON'T remap slots with `colour --rgb` any more: that only paints
# on a framebuffer console, and when the framebuffer didn't engage the whole menu
# fell back to an unreadable cyan-on-cyan mess.  Using the stock slots directly
# means the menu looks right regardless of whether the framebuffer is active.
#   norm_fg/norm_bg = default text;  sel_fg/sel_bg = highlighted item.
_THEME_COLOURS = {
    # Dark: white text on black, cyan selection bar with black text.
    "dark":  {"norm_fg": 7, "norm_bg": 0, "sel_fg": 0, "sel_bg": 6},
    # Light: black text on white, blue selection bar with white text.
    "light": {"norm_fg": 0, "norm_bg": 7, "sel_fg": 7, "sel_bg": 4},
}
...
def _header(server_ip: str, title: str, theme: str = "dark") -> str:
    ...
    # lacking a feature the script still falls through to the menu instead of
    # aborting.
    return f"""#!ipxe
...
# --- Colour scheme using the fixed ANSI slots (work in plain VGA text mode).
# Default text pair (0) and the highlighted-item pair (1) for the {theme} theme.
cpair --foreground {c['norm_fg']} --background {c['norm_bg']} 0 || true
cpair --foreground {c['sel_fg']} --background {c['sel_bg']} 1 || true
"""
